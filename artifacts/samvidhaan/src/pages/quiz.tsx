...
export default function Quiz() {
  ...
  if (!fbUser) {
    return (
      <div className="min-h-[calc(100vh-10rem)] bg-background py-16 px-4 md:px-8 flex items-center justify-center">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="max-w-md w-full text-center space-y-8 p-10 bg-card border border-border/40 rounded-[2.5rem] shadow-2xl shadow-primary/5"
        >
          <div className="mx-auto w-20 h-20 rounded-3xl bg-primary/10 flex items-center justify-center text-primary mb-6">
            <Award className="h-10 w-10" />
          </div>
          <div>
            <h1 className="text-3xl font-black font-serif text-foreground tracking-tight mb-3">Login to Compete</h1>
            <p className="text-muted-foreground text-sm leading-relaxed">
              Sign in to attempt quizzes, track your accuracy, and climb the ranks of constitutional scholars.
            </p>
          </div>
          <Button 
